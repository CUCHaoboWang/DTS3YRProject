...
import { VideoSettings } from '../shared/types/video-settings';
...
export class WebComponentComponent {
  ...
  @ViewChild('videoConferencing') videoConferencing: VideoConferencingComponent;
  ...
  set sessionConfig(config: string | ISessionConfig | Object) {
    ...
    setTimeout(() => {
      ...
      if (this.isEmpty(config)) {
        ...
        this.videoConferencing?.leaveSession();
        return;
      }
  ...
  emitSession(session: any) {
    ...
    session.on('connectionCreated', (e: ConnectionEvent) => {
      this.videoConferencing.checkSizeComponent();
    });
